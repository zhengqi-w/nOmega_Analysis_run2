...
def WriteTree(df, cols, treeName, fileName):
    ...
    outBranches = {}
    for colName in cols:
        outBranches[colName] = np.float64 #define all branches as float for the moment
    with uproot3.recreate(fileName, compression=uproot3.LZ4(4)) as outFile:
        outFile[treeName] = uproot3.newtree(outBranches, compression=uproot3.LZ4(4))
...
def data_prep(inputCfg, iBin, PtBin, OutPutDirPt, PromptDf, FDDf, BkgDf): #pylint: disable=too-many-statements, too-many-branches
    ...
    dataset_opt = inputCfg['data_prep']['dataset_opt'] # 'max_signal' -> try to use all the signal (prompt and FD) + add n_bkg = bkg_mult * (n_prompt + n_FD)
    ...
    if dataset_opt == 'equal':
        ...
        if test_f < 1:
            TrainSet, TestSet, yTrain, yTest = train_test_split(TotDf, LabelsArray, test_size=test_f, random_state=seed_split)
        else:
        ...
        sys.exit()
    print("TrainSet (first 5 rows):")
    print(TrainSet.head(5))
    print("\nTestSet (first 5 rows):")
    ...
    for Fig, Lab in zip(CorrMatrixFig, OutputLabels):
        ...
        Fig.savefig(f'{OutPutDirPt}/CorrMatrix{Lab}_pT_{PtBin[0]}_{PtBin[1]}.png')
...
def train_test(inputCfg, PtBin, OutPutDirPt, TrainTestData, iBin): #pylint: disable=too-many-statements, too-many-branches
    ...
    '''
    n_classes = len(np.unique(TrainTestData[3])) # 去除数组中的重复数字，并进行排序之后输出。(等于2)
    ...
    if inputCfg['ml']['hyper_par_opt']['do_hyp_opt']:
        ...
        ModelHandl.optimize_params_optuna(TrainTestData, BayesOptConfig, cross_val_scoring='roc_auc_ovo', nfold=inputCfg['ml']['hyper_par_opt']['nfolds'], direction='maximize')
#        ModelHandl.optimize_params_optuna(TrainTestData, BayesOptConfig, metric,
...
#                                          njobs=inputCfg['ml']['hyper_par_opt']['njobs'])
    ...
    #_____________________________________________
    LearningCurvesFig = plot_utils.plot_learning_curves(ModelHandl, TrainTestData)
    LearningCurvesFig.savefig(f'{OutPutDirPt}/LearningCurves_pT_{PtBin[0]}_{PtBin[1]}.pdf')
...
def appl(inputCfg, PtBin, OutPutDirPt, ModelHandl, DataDfPtSel, PromptDfPtSelForEff, FDDfPtSelForEff):
    ...
    if not isinstance(df_column_to_save_list, list):
        ...
        sys.exit()
    if 'mass_Omegac0' not in df_column_to_save_list:
        print('\033[93mWARNING: mass_Omegac0 is not going to be saved in the output dataframe!\033[0m')
    if 'pt_Omegac0' not in df_column_to_save_list:
        print('\033[93mWARNING: pt_Omegac0 is not going to be saved in the output dataframe!\033[0m')
    PromptDfPtSelForEff = PromptDfPtSelForEff.loc[:, df_column_to_save_list]
    if FDDfPtSelForEff.empty:
        out = 'Signal'
        PromptDfPtSelForEff['MLscore'] = yPredPromptEff
    else:
        out = 'Prompt'
    ...
    WriteTree(PromptDfPtSelForEff, ['mass_Omegac0', 'pt_Omegac0', 'pt_PiFromOmegac0', 'MLscore'], 'tree_ML', f'{OutPutDirPt}/{out}_pT_{PtBin[0]}_{PtBin[1]}_ModelApplied.root')
    ...
    if not FDDfPtSelForEff.empty:
        ...
        WriteTree(FDDfPtSelForEff, ['mass_Omegac0', 'pt_Omegac0', 'pt_PiFromOmegac0', 'MLscore'], 'tree_ML', f'{OutPutDirPt}/{out}_pT_{PtBin[0]}_{PtBin[1]}_ModelApplied.root')
    ...
    WriteTree(DataDfPtSel, ['mass_Omegac0', 'pt_Omegac0', 'pt_PiFromOmegac0', 'MLscore'], 'tree_ML', f'{OutPutDirPt}/Data_pT_{PtBin[0]}_{PtBin[1]}_ModelApplied.root')
...
def main(): #pylint: disable=too-many-statements
    ...
    PromptHandler.slice_data_frame('pt_Omegac0', PtBins, True)
    if FDHandler is not None:
        FDHandler.slice_data_frame('pt_Omegac0', PtBins, True)
    DataHandler.slice_data_frame('pt_Omegac0', PtBins, True)
    BkgHandler.slice_data_frame('pt_Omegac0', PtBins, True)
